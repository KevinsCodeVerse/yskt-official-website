<template>
  <div>
    <div
      ...
      class="yb-video"
      :prop="videoProp"
      :change:prop="propWatcher"
    >
      <div class="ybplayer-slots">
        <div class="ybplayer-controls-slots">
          <slot name="controls"></slot>
        </div>
        <div class="ybplayer-default-slots">
          <slot></slot>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    initDom() {
      myVideo = document.getElementById("yb-video" + this.videoProp.dataId)
      console.dir(myVideo);
    ...
    propWatcher(newVal, oldVal = {}) {
      ...
      if (this.dp) {
        ...
        if (
          newVal.barrages.length > 0 &&
          oldVal.barrages &&
          oldVal.barrages.length == 0
        ) {
...
<style>
@import url("../modules/ybplayer.css");
.yb-video {
  position: relative;
  overflow: hidden;
  width: 100%;
